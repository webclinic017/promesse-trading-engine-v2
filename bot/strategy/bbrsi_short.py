...
class BBRSI(Strategy):
    ...
    def calculate_signals(self):
        for symbol in self.symbol_list:
            ...
            ma_long = SMA(closes, timeperiod=200)
            ma_short = SMA(closes, timeperiod=50)

            # Buy Signal conditions
            if self.position[symbol] == 'OUT':
                trend = detect_div(highs, rsis)

                if trend == 'bear' and ma_long[-1] > ma_short[-1]:
                ...
                if trend != 'bear' and self.is_reversal[symbol] != 0:
                    self.is_reversal[symbol] = 0
                    break
